...
Employee cells use single click for focus and double click for mouse editing. Existing keyboard editing remains available. Employee management now opts into the Community range-paste helper: drag a rectangle and paste Excel values using Ctrl+V. Add new rows through Input before pasting; clipboard paste no longer implicitly inserts employees. Other screens do not opt in.

Paste follows displayed data-column order and current filtered/sorted row order. Hidden columns and action/status columns do not consume source positions. Locked data cells consume their source position but remain unchanged. One source cell fills a rectangle; a table must match the rectangle or expand from a single anchor. Reject page/column overflow and malformed tables without truncation. Validate every writable target before applying one local row batch. Department, position, employment status, active flag and date conversion remain screen-owned; shared dirty-row reconciliation preserves original snapshots. Invalid values abort the paste. Search inputs and active cell editors retain native text paste. Sorting, filtering, data reload and column visibility/order changes clear selection. Save permission is required; loading and saving block range writes.

The implementation supports one rectangle on the current page. Ctrl+C copies formatted values as Excel-compatible text with quoted tabs/newlines; password input values are excluded. The bottom-right fill handle extends down or right, repeats text, increments dates, and extrapolates a two-number series. Password columns cannot be auto-filled. Multi-range selection and automatic scrolling at viewport edges are not included. The existing Save action persists changes. Do not activate these helpers on other screens until their field conversion and permission rules have been reviewed.

Employee grid tools remain opt-in and Community-only:
- Undo/redo stores up to 30 in-memory row snapshots. Each paste/fill is one operation; individual cell edits, input, row-copy, upload and deletion also use the same local history. Save success and accepted reload clear history; canceled reload and failed save preserve it. Ctrl+Z/Ctrl+Y apply outside native input editors. History is never persisted.
- A keyboard-accessible context menu offers range copy/paste, undo/redo, the clicked employee's detail and selected-row deletion through the existing confirmation. Clipboard permission failure directs the user to Ctrl+C/Ctrl+V. Mutation actions respect saving/loading and menu permissions.
- Selection statistics show selected cells, nonempty values, numeric count, sum and average. Numeric IDs are excluded; text and date cells are not coerced to numbers. Employee fields normally have no numeric measures, so numeric aggregates display a dash.
- Department, position, employment status and account-active columns have searchable check-list filters with select-all/deselect-all. Values and filtering apply to the loaded page, and option lists refresh with row data and popup opening. Existing top search remains the server query boundary.
- Analysis opens a read-only snapshot of the current page's filtered/sorted rows, including unsaved edits and excluding deleted rows. Groups expand to employees and subtotal counts; pivot axes select department, position or employment status with row/column totals; the bar chart shows group counts with text alternatives. Every view states its scope and count. These are current-page tools, not company-wide analytics or the Enterprise row model.
