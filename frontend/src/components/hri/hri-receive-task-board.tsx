...
export function HriReceiveTaskBoard() {
  ...
  async function loadTasks() {
    setLoading(true);
    try {
      const res = await fetch("/api/hri/tasks/my-receives", { cache: "no-store" });
      const raw = (await res.json().catch(() => null)) as unknown;
      if (!res.ok) {
        const errorData = raw as { detail?: string } | null;
        throw new Error(errorData?.detail ?? "수신함 조회에 실패했습니다.");
      }
      ...
      setNoticeType("error");
      setNotice(error instanceof Error ? error.message : "수신함 조회에 실패했습니다.");
    } finally {
      setLoading(false);
    }
  }

  async function runAction(action: "receive-complete" | "receive-reject", requestId: number) {
    setActing(true);
    setNotice(null);
    try {
      ...
      setNotice(action === "receive-complete" ? "수신 처리 완료" : "수신 반려 완료");
      setComment("");
      await loadTasks();
    } catch (error) {
      setNoticeType("error");
  ...
  if (loading) {
    return <div className="p-6 text-sm text-muted-foreground">수신함을 불러오는 중입니다...</div>;
  }
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>수신 처리 대기 목록</CardTitle>
        </CardHeader>
        <CardContent>
                    ...
                    <td className="border px-2 py-2">
                      <div className="flex justify-center gap-1">
                        <Button
                          size="xs"
                          variant="action"
                          onClick={() => void runAction("receive-complete", item.request_id)}
                          disabled={acting}
                        >
                          처리완료
                        </Button>
                        <Button
                          size="xs"
                          variant="destructive"
                          onClick={() => void runAction("receive-reject", item.request_id)}
                          disabled={acting}
                        >
                          수신반려
                        </Button>
