...
type ApprovalTaskGridRow = HriTaskItem & ReadonlyGridRow;
...
export function HriApprovalTaskBoard() {
  ...
  const query = useMemo(() => {
    ...
    return `/api/hri/tasks/my-approvals?${params.toString()}`;
  }, [page]);
  ...
  const rowData = useMemo<ApprovalTaskGridRow[]>(
    () => createReadonlyGridRows(filteredItems.map((item) => ({ ...item, id: item.request_id }))),
    [filteredItems],
  );

  const runAction = useCallback(async (action: "approve" | "reject", requestId: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json && "detail" in json ? json.detail ?? "결재 처리에 실패했습니다." : "결재 처리에 실패했습니다.");
      }

      setNoticeTone("success");
      setNotice(action === "approve" ? "결재 승인이 완료되었습니다." : "결재 반려가 완료되었습니다.");
      setComment("");
      await mutate();
    } catch (error) {
      setNoticeTone("error");
      setNotice(error instanceof Error ? error.message : "결재 처리에 실패했습니다.");
    } finally {
      setActingRequestId(null);
    }
  }, [comment, mutate]);

  const columnDefs = useMemo<ColDef<ApprovalTaskGridRow>[]>(
    () => [
      ...
      },
      {
        ...
        sortable: false,
        filter: false,
        cellRenderer: (params: { data?: ApprovalTaskGridRow }) => {
          if (!params.data) return null;

          const disabled = actingRequestId !== null && actingRequestId === params.data.request_id;
          return (
            <div className="flex h-full items-center gap-1">
              <Button size="sm" onClick={() => void runAction("approve", params.data!.request_id)} disabled={disabled}>
                승인
              </Button>
              <Button
                size="sm"
                variant="outline"
                onClick={() => void runAction("reject", params.data!.request_id)}
                disabled={disabled}
              >
                반려
              </Button>
  ...
  return (
    <ReadonlyGridManager<ApprovalTaskGridRow>
      title="결재함"
      searchFields={
