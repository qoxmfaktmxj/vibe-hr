...
const STATUS_LABELS: Record<GridRowStatus, string> = { clean: "", added: "입력", updated: "수정", deleted: "삭제" };
const SOURCE_LABELS = { if: "IF", manual: "수기" } as const;
const HIRE_TYPE_LABELS = { new: "신입", experienced: "경력" } as const;
const PROCESS_STATUS_LABELS = { draft: "등록", ready: "발령대기", appointed: "발령완료" } as const;
const AG_GRID_LOCALE_KO: Record<string, string> = { page: "페이지", more: "더보기", to: "~", of: "/", next: "다음", last: "마지막", first: "처음", previous: "이전", loadingOoo: "로딩 중...", noRowsToShow: "데이터가 없습니다.", searchOoo: "검색..." };
...
export function HrRecruitFinalistManager() {
  ...
  const downloadTemplate = useCallback(async () => {
    try {
      ...
      const sheet = utils.aoa_to_sheet([["이름", "주민등록번호(마스킹)", "생년월일", "휴대전화", "이메일", "입사유형(new/experienced)", "경력연수", "예정입사일", "비고"], ["홍길동", "900101-1******", "1990-01-01", "010-1234-5678", "hong@example.com", "new", "0", "2026-04-01", "신입 채용"]]);
  ...
  const downloadCurrentSheet = useCallback(async () => {
    try {
      ...
      const sheet = utils.aoa_to_sheet([["상태", "합격자번호", "등록구분", "이름", "주민번호", "생년월일", "휴대전화", "이메일", "입사유형", "경력연수", "로그인ID", "사번", "예정입사일", "진행상태", "비고"], ...rowsForExport]);
      const book = utils.book_new();
  ...
  const saveAll = useCallback(async () => {
    ...
    try {
      ...
      if (toDelete.length > 0) {
        ...
        if (!response.ok) throw new Error(await parseErrorDetail(response, "저장에 실패했습니다."));
      }
      for (const row of toUpdate) {
        ...
        if (!response.ok) throw new Error(await parseErrorDetail(response, `${row.full_name} 저장에 실패했습니다.`));
      }
      ...
      await mutate();
      toast.success(`저장이 완료되었습니다. (입력 ${toInsert.length}건 / 수정 ${toUpdate.length}건 / 삭제 ${toDelete.length}건)`);
    } catch (error) {
  ...
  const generateEmployeeNoForSelected = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await parseErrorDetail(response, "사번 생성에 실패했습니다."));
      await mutate();
      toast.success("사번 생성이 완료되었습니다.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "사번 생성에 실패했습니다.");
    } finally {
  ...
  const columnDefs = useMemo<ColDef<RecruitGridRow>[]>(() => [
    ...
    { field: "hire_type", headerName: "입사유형", minWidth: 110, editable: (params) => params.data?._status !== "deleted", cellEditor: "agSelectCellEditor", cellEditorParams: { values: ["new", "experienced"] }, valueFormatter: (params) => HIRE_TYPE_LABELS[(params.value as keyof typeof HIRE_TYPE_LABELS) ?? "new"] },
    { field: "career_years", headerName: "경력연수", minWidth: 100, editable: (params) => params.data?._status !== "deleted" },
  ...
  const toolbarSaveAction = { key: "save", label: saving ? "저장..." : "저장", icon: Save, onClick: () => void saveAll(), disabled: isLoading || saving || generatingNo, variant: "save" as const };
  ...
  return (
    <ManagerPageShell>
      <ManagerSearchSection title="채용합격자등록" onQuery={handleQuery} queryLabel="조회" queryDisabled={isLoading || saving || generatingNo}>
        <SearchFieldGrid className="xl:grid-cols-3">
          <SearchTextField value={searchFilters.keyword} onChange={(value) => setSearchFilters((prev) => ({ ...prev, keyword: value }))} onKeyDown={handleSearchEnter} placeholder="이름 / 합격자번호 / 사번 / 로그인ID" />
      ...
      <ManagerGridSection
        ...
        headerRight={<><GridToolbarActions actions={toolbarActions} saveAction={toolbarSaveAction} /><Button size="sm" variant="action" onClick={() => void syncFromIf()} disabled><RefreshCw className="h-3.5 w-3.5" />IF 수신 준비중</Button><Button size="sm" variant="action" onClick={() => void generateEmployeeNoForSelected()} disabled={isLoading || saving || generatingNo}><UserRoundPlus className="h-3.5 w-3.5" />{generatingNo ? "사번생성중..." : "사번생성"}</Button><input ref={uploadInputRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleUploadFile(file); event.currentTarget.value = ""; }} /></>}
      ...
      >
        <div className="min-h-0 flex-1 px-3 pb-4 pt-2 md:px-6 md:pt-0">
          <div className="ag-theme-quartz vibe-grid h-full w-full min-h-[420px] overflow-hidden rounded-lg border border-gray-200">
            <AgGridReact<RecruitGridRow> theme="legacy" rowData={pagedRows} columnDefs={columnDefs} defaultColDef={defaultColDef} rowSelection="multiple" suppressRowClickSelection={false} singleClickEdit animateRows={false} localeText={AG_GRID_LOCALE_KO} rowClassRules={rowClassRules} getRowClass={getRowClass} getRowId={(params) => String(params.data.id)} onGridReady={(event: GridReadyEvent<RecruitGridRow>) => { gridApiRef.current = event.api; }} onCellValueChanged={onCellValueChanged} loading={isLoading} headerHeight={36} rowHeight={34} overlayNoRowsTemplate='<span class="text-sm text-slate-400">데이터가 없습니다.</span>' />
          </div>
        </div>
      </ManagerGridSection>

      <ConfirmDialog open={discardDialogOpen} onOpenChange={handleDiscardDialogOpenChange} title="저장되지 않은 변경 사항이 있습니다." description="현재 변경 내용을 저장하지 않고 이동하면 수정 내용이 사라집니다. 계속 진행하시겠습니까?" confirmLabel="무시하고 이동" cancelLabel="취소" confirmVariant="destructive" onConfirm={handleDiscardAndContinue} />
    </ManagerPageShell>
  );
}
