...
import { reconcileUpdatedStatus, toggleDeletedStatus, clearSavedStatuses } from "@/lib/grid/grid-status-mutations";
...
} from "@/types/pap-final-result";

type RowStatus = GridRowStatus;

type PapFinalResultRow = PapFinalResultItem & {
  _status: RowStatus;
...
const TRACKED_FIELDS: (keyof PapFinalResultItem)[] = [
  "result_code",
  "result_name",
  "score_grade",
  "is_active",
...
const I18N = {
  title: "최종등급관리",
  query: "조회",
  loading: "최종등급 데이터를 불러오는 중...",
  loadError: "최종등급 목록을 불러오지 못했습니다.",
  saveDone: "저장이 완료되었습니다.",
  saveFail: "저장에 실패했습니다.",
  noRows: "최종등급 데이터가 없습니다.",
  addRow: "입력",
  copy: "복사",
...
function snapshotOriginal(row: PapFinalResultItem): Record<string, unknown> {
  return snapshotFields(row, TRACKED_FIELDS);
}

function toGridRow(row: PapFinalResultItem): PapFinalResultRow {
  return { ...row, _status: "clean", _original: snapshotOriginal(row), _prevStatus: undefined };
}

function isRevertedToOriginal(row: PapFinalResultRow): boolean {
  return isRowRevertedToOriginal(row, TRACKED_FIELDS);
}

function createEmptyRow(tempId: number): PapFinalResultRow {
...
export function PapFinalResultManager() {
  const { can, loading: menuActionLoading } = useMenuActions("/pap/final-results");
  const [rows, setRows] = useState<PapFinalResultRow[]>([]);
  ...
  const gridApiRef = useRef<GridApi<PapFinalResultRow> | null>(null);
  const uploadInputRef = useRef<HTMLInputElement | null>(null);
  const tempIdRef = useRef(-1);
  const rowsRef = useRef<PapFinalResultRow[]>([]);
  ...
  const rowClassRules = useMemo(() => buildGridRowClassRules<PapFinalResultRow>(), []);

  const getRowKey = useCallback((row: PapFinalResultRow) => String(row.id), []);

  const applyGridTransaction = useCallback((prevRows: PapFinalResultRow[], nextRows: PapFinalResultRow[]) => {
    const api = gridApiRef.current;
    if (!api) return;

    const prevMap = new Map(prevRows.map((row) => [getRowKey(row), row]));
    const nextMap = new Map(nextRows.map((row) => [getRowKey(row), row]));
    const add: PapFinalResultRow[] = [];
    const update: PapFinalResultRow[] = [];
    const remove: PapFinalResultRow[] = [];
  ...
  const commitRows = useCallback((updater: (prevRows: PapFinalResultRow[]) => PapFinalResultRow[]) => {
  ...
  const fetchRows = useCallback(async (filters: SearchFilters, pageNo: number) => {
    ...
    const response = await fetch(`/api/pap/final-results?${params.toString()}`, { cache: "no-store" });
    if (!response.ok) {
      throw new Error(await parseErrorDetail(response, I18N.loadError));
    }
    return (await response.json()) as PapFinalResultListResponse;
  }, [pageSize]);
  ...
  const patchRow = useCallback((rowId: number, patch: Partial<PapFinalResultRow>) => {
    commitRows((prev) =>
      prev.map((row) => {
        if (row.id !== rowId) return row;
        const merged: PapFinalResultRow = { ...row, ...patch, _original: row._original, _prevStatus: row._prevStatus };
  ...
  const copyRow = useCallback(() => {
    ...
    const now = new Date().toISOString();
    const clone: PapFinalResultRow = {
      ...selectedRow,
      id: issueTempId(),
      result_code: `${selectedRow.result_code}_COPY`,
      created_at: now,
      updated_at: now,
      _status: "added",
      _original: undefined,
  ...
  const saveAll = useCallback(async () => {
    ...
    for (const row of [...toInsert, ...toUpdate]) {
      if (!row.result_code.trim()) {
        toast.error("등급코드는 필수입니다.");
        return;
      }
      if (!row.result_name.trim()) {
        toast.error("등급명은 필수입니다.");
        return;
      }
    }
    ...
    try {
      await runConcurrentOrThrow(
        ...
        toDelete.map((row) => async () => {
          const response = await fetch(`/api/pap/final-results/${row.id}`, { method: "DELETE" });
          if (!response.ok) throw new Error(await parseErrorDetail(response, `${row.result_code} 삭제 실패`));
        }),
        6,
      );
      ...
      await runConcurrentOrThrow(
        ...
        toUpdate.map((row) => async () => {
          const response = await fetch(`/api/pap/final-results/${row.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              result_code: row.result_code.trim(),
              result_name: row.result_name.trim(),
              score_grade: row.score_grade != null ? Number(row.score_grade) : null,
              is_active: row.is_active,
              sort_order: Number(row.sort_order || 0),
              description: row.description || null,
            }),
          });
          if (!response.ok) throw new Error(await parseErrorDetail(response, `${row.result_code} 수정 실패`));
        }),
        6,
      );
      ...
      await runConcurrentOrThrow(
        ...
        toInsert.map((row) => async () => {
          const response = await fetch("/api/pap/final-results", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              result_code: row.result_code.trim(),
          ...
          if (!response.ok) throw new Error(await parseErrorDetail(response, `${row.result_code || "(신규)"} 입력 실패`));
        }),
        6,
      );
  ...
  const onCellValueChanged = useCallback((event: CellValueChangedEvent<PapFinalResultRow>) => {
    if (event.newValue === event.oldValue) return;
    const rowId = event.data?.id;
    const field = event.colDef.field as keyof PapFinalResultRow | undefined;
    if (rowId == null || !field) return;

    commitRows((prev) =>
      prev.map((row) => {
        if (row.id !== rowId) return row;
        const next = { ...row } as PapFinalResultRow;
        if (field === "result_code" || field === "result_name" || field === "description") {
          next[field] = String(event.newValue ?? "") as never;
        } else if (field === "score_grade") {
          const parsed = String(event.newValue ?? "").trim();
          next.score_grade = parsed ? Number(parsed) : null;
  ...
  const columnDefs = useMemo<ColDef<PapFinalResultRow>[]>(() => [
    {
      headerName: "삭제",
      width: 56,
      pinned: "left",
      ...
      editable: false,
      cellRenderer: (params: ICellRendererParams<PapFinalResultRow>) => {
  ...
  ], [patchRow, toggleDeleteById]);

  const defaultColDef = useMemo<ColDef<PapFinalResultRow>>(
    () => ({ sortable: true, filter: true, resizable: true, editable: false }),
    [],
  );

  const getRowClass = useCallback((params: RowClassParams<PapFinalResultRow>) => {
    return getGridRowClass(params.data?._status);
  }, []);

  const onGridReady = useCallback((event: GridReadyEvent<PapFinalResultRow>) => {
  ...
  async function downloadTemplate() {
    try {
      ...
      const sheet = utils.aoa_to_sheet([
        ["등급코드", "등급명", "점수등급", "정렬순서", "사용(Y/N)", "설명"],
        ["A", "우수", "90", "10", "Y", "상위 평가 등급"],
      ]);
      const book = utils.book_new();
      utils.book_append_sheet(book, sheet, "최종등급");
      writeFileXLSX(book, "pap-final-results-template.xlsx");
    } catch {
  ...
  async function downloadXlsx() {
    try {
      ...
      const book = utils.book_new();
      utils.book_append_sheet(book, sheet, "최종등급관리");
      writeFileXLSX(book, `pap-final-results-${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch {
  ...
  async function handleUploadFile(file: File) {
    try {
      ...
      const parsed = rowsAoa.slice(1)
        ...
        .map<PapFinalResultRow>((cells) => ({
          ...createEmptyRow(issueTempId()),
          result_code: cells[0] ?? "",
          result_name: cells[1] ?? "",
          score_grade: cells[2] ? Number(cells[2]) : null,
          sort_order: Number(cells[3] || 0),
          is_active: ["y", "true", "1"].includes((cells[4] ?? "").toLowerCase()),
          description: cells[5] ?? "",
        }));
