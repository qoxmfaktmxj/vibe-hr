...
type GlAccountRow = GlAccountItem & {
  _status: RowStatus;
...
const ACCOUNT_TYPE_LABELS: Record<string, string> = {
  expense: "비용", liability: "부채", asset: "자산", equity: "자본", revenue: "수익",
};
...
/* 보조 함수                                                           */
/* ------------------------------------------------------------------ */
function snapshotOriginal(row: GlAccountItem): Record<string, unknown> {
  return snapshotFields(row, TRACKED_FIELDS);
}

function isReverted(row: GlAccountRow): boolean {
  return isRowRevertedToOriginal(row, TRACKED_FIELDS);
}

function toGridRow(item: GlAccountItem): GlAccountRow {
  return { ...item, _status: "clean", _original: snapshotOriginal(item) };
}

function createEmptyRow(tempId: number): GlAccountRow {
...
/* 컴포넌트                                                            */
/* ------------------------------------------------------------------ */
export function PayGlAccountManager() {
  const { can, loading: menuActionLoading } = useMenuActions("/payroll/gl-accounts");
  ...
  const gridApiRef = useRef<GridApi<GlAccountRow> | null>(null);
  const tempIdRef = useRef(-1);
  const rowsRef = useRef<GlAccountRow[]>([]);
  const pageSize = 100;
  ...
  const rowClassRules = useMemo(() => buildGridRowClassRules<GlAccountRow>(), []);

  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/pay/gl-accounts", { cache: "no-store" });
      if (!res.ok) throw new Error("계정과목 목록을 불러오지 못했습니다.");
      const data = (await res.json()) as { items: GlAccountItem[] };
  ...
  const getRowKey = useCallback((row: GlAccountRow) => String(row.id), []);

  const applyGridTransaction = useCallback(
    (prevRows: GlAccountRow[], nextRows: GlAccountRow[]) => {
      const api = gridApiRef.current;
      if (!api) return;

      const prevMap = new Map(prevRows.map((row) => [getRowKey(row), row]));
      const nextMap = new Map(nextRows.map((row) => [getRowKey(row), row]));
      const add: GlAccountRow[] = [];
      const update: GlAccountRow[] = [];
      const remove: GlAccountRow[] = [];
  ...
  const commitRows = useCallback(
    (updater: (prevRows: GlAccountRow[]) => GlAccountRow[]) => {
  ...
  const columnDefs = useMemo<ColDef<GlAccountRow>[]>(
    () => [
      {
        headerName: "삭제",
        width: 52,
        ...
        resizable: false,
        editable: false,
        cellRenderer: (params: ICellRendererParams<GlAccountRow>) => {
      ...
      },
      {
        ...
        cellEditorParams: { values: ACCOUNT_TYPE_OPTIONS },
        valueFormatter: (p) => ACCOUNT_TYPE_LABELS[p.value as string] ?? (p.value as string),
      },
      {
        headerName: "실지급계정",
        field: "is_net_pay_account",
        width: 100,
        editable: (p) => p.data?._status !== "deleted",
        cellEditor: "agSelectCellEditor",
  ...
  const defaultColDef = useMemo<ColDef<GlAccountRow>>(
    () => ({ sortable: true, filter: true, resizable: true, editable: false }),
    [],
  );
  const selectionColumnDef = useMemo<ColDef<GlAccountRow>>(
    () => ({
      width: 52,
      pinned: "left",
      sortable: false,
  ...
  const onGridReady = useCallback((e: GridReadyEvent<GlAccountRow>) => {
    gridApiRef.current = e.api;
  }, []);

  const onCellValueChanged = useCallback(
    (e: CellValueChangedEvent<GlAccountRow>) => {
      ...
      commitRows((prev) =>
        prev.map((row) => {
          ...
          return reconcileUpdatedStatus({ ...row } as GlAccountRow, {
            shouldBeClean: (candidate) => isReverted(candidate),
          });
        }),
      );
  ...
  async function downloadExcel() {
    try {
      ...
      utils.book_append_sheet(book, sheet, "계정과목");
      writeFileXLSX(book, `pay-gl-accounts-${new Date().toISOString().slice(0, 10)}.xlsx`);
    } catch {
  ...
  async function saveAllChanges() {
    ...
    for (const r of [...toInsert, ...toUpdate]) {
      if (!r.code.trim()) { toast.error(`계정코드를 입력해 주세요. (행 ID: ${r.id})`); return; }
      if (!r.name.trim()) { toast.error(`계정명을 입력해 주세요. (코드: ${r.code})`); return; }
    }

    setSaving(true);
    try {
      const payload: GlAccountBatchRequest = {
      ...
      const res = await fetch("/api/pay/gl-accounts/batch", {
        method: "POST",
      ...
      const json = (await res.json()) as GlAccountBatchResponse;
