...
type RowData = PayAllowanceDeductionItem & {
    _status: RowStatus;
    _original?: Record<string, unknown>;
    _prevStatus?: RowStatus;
};

const TRACKED_FIELDS: (keyof PayAllowanceDeductionItem)[] = [
    "code", "name", "type", "tax_type", "calculation_type", "is_active", "sort_order"
];
...
const TYPE_OPTIONS = ["allowance", "deduction"];
const TYPE_LABELS: Record<string, string> = { allowance: "지급(수당)", deduction: "공제" };

const TAX_TYPE_OPTIONS = ["taxable", "non-taxable", "tax", "insurance"];
const TAX_TYPE_LABELS: Record<string, string> = { taxable: "과세", "non-taxable": "비과세", tax: "세금", insurance: "사대보험" };

const CALCULATION_TYPE_OPTIONS = ["fixed", "hourly", "formula"];
const CALCULATION_TYPE_LABELS: Record<string, string> = { fixed: "고정액", hourly: "시급 비례", formula: "계산식" };
...
function snapshotOriginal(row: PayAllowanceDeductionItem): Record<string, unknown> {
...
function toGridRow(item: PayAllowanceDeductionItem): RowData {
...
export function PayAllowanceDeductionManager() {
    const { can, loading: menuActionLoading } = useMenuActions("/payroll/allowance-deduction-items");
    ...
    const loadData = useCallback(async () => {
        setLoading(true);
        try {
            const res = await fetch("/api/pay/setup/allowance-deductions", { cache: "no-store" });
            if (!res.ok) throw new Error("항목 목록을 불러오지 못했습니다.");
            const data = (await res.json()) as { items: PayAllowanceDeductionItem[] };
    ...
    const columnDefs = useMemo<ColDef<RowData>[]>(
        () => [
            ...
            },
            {
                ...
                valueFormatter: (p) => TYPE_LABELS[p.value as string] ?? p.value,
            },
            {
                headerName: "과세유형",
                field: "tax_type",
                ...
                editable: (p) => p.data?._status !== "deleted",
                cellEditor: "agSelectCellEditor",
                cellEditorParams: { values: TAX_TYPE_OPTIONS },
                valueFormatter: (p) => TAX_TYPE_LABELS[p.value as string] ?? p.value,
            },
            {
                headerName: "계산방식",
                ...
                editable: (p) => p.data?._status !== "deleted",
                cellEditor: "agSelectCellEditor",
                cellEditorParams: { values: CALCULATION_TYPE_OPTIONS },
                valueFormatter: (p) => CALCULATION_TYPE_LABELS[p.value as string] ?? p.value,
            },
            {
                headerName: "정렬순서",
                field: "sort_order",
    ...
    async function downloadExcel() {
        try {
            const headers = ["코드", "이름", "항목유형", "과세유형", "계산방식", "사용여부", "정렬순서"];
            const data = rows
                .filter((r) => r._status !== "deleted")
                .map((r) => [
                    r.code, r.name, TYPE_LABELS[r.type] ?? r.type, TAX_TYPE_LABELS[r.tax_type] ?? r.tax_type,
                    CALCULATION_TYPE_LABELS[r.calculation_type] ?? r.calculation_type,
                    r.is_active ? "Y" : "N", r.sort_order
                ]);
            const { utils, writeFileXLSX } = await import("xlsx");
            const sheet = utils.aoa_to_sheet([headers, ...data]);
            const book = utils.book_new();
            utils.book_append_sheet(book, sheet, "수당공제항목");
            writeFileXLSX(book, `allowance-deduction-items-${new Date().toISOString().slice(0, 10)}.xlsx`);
        } catch {
    ...
    async function saveAllChanges() {
        ...
        for (const r of [...toInsert, ...toUpdate]) {
            if (!r.code.trim()) { toast.error(`코드를 입력해 주세요. (행 ID: ${r.id})`); return; }
            if (!r.name.trim()) { toast.error(`이름을 입력해 주세요. (코드: ${r.code})`); return; }
        }

        setSaving(true);
        try {
            const payload: PayAllowanceDeductionBatchRequest = {
            ...
            const res = await fetch("/api/pay/setup/allowance-deductions/batch", {
                method: "POST",
            ...
            const json = (await res.json()) as PayAllowanceDeductionBatchResponse;
