...
export function PayPaymentScheduleManager() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("월급여일자 데이터를 불러오지 못했습니다.");
  ...
  async function saveAllChanges() {
    const items = rows
      ...
      .map((row) => ({
        ...
        payroll_code_id: Number(row.payroll_code_id),
    ...
    try {
      ...
      toast.success(`저장 완료 (수정 ${json.updated_count}건)`);
    } catch (error) {
  ...
  const columnDefs = useMemo<ColDef<RowData>[]>(
    () => [
      ...
      { headerName: "급여코드", field: "payroll_code_name", width: 130, editable: false },
      {
        headerName: "지급일유형",
        ...
        headerRight={
          <GridToolbarActions
            actions={[
              ...
              { key: "upload", label: "업로드", icon: Search, onClick: () => toast.success("업로드는 다음 단계에서 연결합니다."), disabled: true },
