...
const TRACKED_FIELDS: (keyof OrgMappingTypeItem)[] = [
  "type_code",
  "item_code",
  "name",
  "effective_from",
...
const I18N = {
  title: "조직구분항목관리",
  loading: "조직구분항목 데이터를 불러오는 중...",
  loadError: "조직구분항목 목록을 불러오지 못했습니다.",
  saveDone: "저장이 완료되었습니다.",
  saveFail: "저장에 실패했습니다.",
  noRows: "조직구분항목 데이터가 없습니다.",
  addRow: "입력",
  copy: "복사",
  download: "다운로드",
  save: "저장",
  ...
  colEffectiveTo: "종료일",
  colErpEmployeeCode: "ERP 사원코드",
  colCostCenterType: "CC유형",
  colSortOrder: "정렬",
  colRemark: "비고",
  colIsActive: "사용여부",
};
...
function snapshotOriginal(row: OrgMappingTypeItem): Record<string, unknown> {
...
function toGridRow(item: OrgMappingTypeItem): RowData {
...
function createSavePayload(row: RowData): MappingTypeItemPayload {
  return {
    type_code: normalizeText(row.type_code),
    ...
    erp_employee_code: normalizeOptionalText(row.erp_employee_code),
    cost_center_type: normalizeOptionalText(row.cost_center_type),
...
const DateCellEditor = forwardRef<
  { getValue: () => string; isPopup: () => boolean },
  DateEditorParams
>(function DateCellEditor(props, ref) {
  const [value, setValue] = useState<string>(
    typeof props.value === "string" ? props.value.slice(0, 10) : "",
  );
...
export function OrgMappingTypeItemManager() {
  ...
  const [appliedFilters, setAppliedFilters] = useState<SearchFilters>({ typeCode: "", referenceDate: "" });
  ...
  const typeCodeOptions = useMemo(() => typeOptions.map((option) => option.code), [typeOptions]);
  ...
  const loadTypeOptions = useCallback(async () => {
    try {
      const response = await fetch("/api/org/mapping-types", { cache: "no-store" });
      if (!response.ok) return;
      const data = (await response.json()) as { items?: MappingTypeOption[] };
  ...
  const fetchMappingTypeItems = useCallback(
    async (filters: SearchFilters, pageNo: number) => {
      ...
      return (await response.json()) as OrgMappingTypeItemListResponse;
    },
    [pageSize],
  );
  ...
  const runQuery = useCallback(
    async (filters: SearchFilters, pageNo: number) => {
      setLoading(true);
      try {
        const response = await fetchMappingTypeItems(filters, pageNo);
  ...
  const columnDefs = useMemo<ColDef<RowData>[]>(() => {
    const typeCodeEditor =
      typeCodeOptions.length > 0
        ? {
            cellEditor: "agSelectCellEditor" as const,
            cellEditorParams: { values: typeCodeOptions },
          }
        : {
    ...
    (event: CellValueChangedEvent<RowData>) => {
      ...
      commitRows((prev) =>
        prev.map((row) => {
          ...
          if (field === "type_code" || field === "item_code" || field === "name") {
            next[field] = normalizeText(event.newValue) as never;
          ...
          } else if (field === "erp_employee_code" || field === "cost_center_type" || field === "remark") {
            next[field] = normalizeOptionalText(event.newValue) as never;
          } else if (field === "sort_order") {
            next.sort_order = normalizeSortOrder(event.newValue);
          } else if (field === "is_active") {
            next.is_active = normalizeBoolean(event.newValue);
          } else {
  ...
  function addRow() {
    ...
    const defaultTypeCode = appliedFilters.typeCode.trim() || typeCodeOptions[0] || "";
    const newRow: RowData = {
      ...
      remark: null,
      sort_order: rowsRef.current.length + 1,
      is_active: true,
      created_at: now,
      updated_at: now,
      _status: "added",
  ...
  function copyRow() {
    ...
    const copied: RowData = {
      ...selectedRow,
      id: newId,
      item_code: `${selectedRow.item_code}_COPY`,
      name: `${selectedRow.name} 복사`,
      created_at: now,
      updated_at: now,
      _status: "added",
      _original: undefined,
  ...
  async function downloadXlsx() {
    const exportRows = rows
      ...
      .map((row, index) => ({
        ...
        "ERP 사원코드": row.erp_employee_code ?? "",
        CC유형: row.cost_center_type ?? "",
        정렬: row.sort_order,
        비고: row.remark ?? "",
        사용여부: row.is_active ? "Y" : "N",
      }));

    const XLSX = await import("xlsx");
    const sheet = XLSX.utils.json_to_sheet(exportRows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, sheet, "조직구분항목");
    XLSX.writeFile(workbook, "org-mapping-type-items.xlsx");
  }
  ...
  async function saveAll() {
    ...
    const invalid = [...toInsert, ...toUpdate].find((row) => {
      return !row.type_code.trim() || !row.item_code.trim() || !row.name.trim() || !row.effective_from.trim();
    });
    if (invalid) {
      toast.error("유형코드 / 항목코드 / 명칭 / 시작일은 필수입니다.");
      return;
    }
    ...
    try {
      for (const row of toDelete) {
        const response = await fetch(`/api/org/mapping-type-items/${row.id}`, { method: "DELETE" });
        if (!response.ok) {
      ...
      >
        <SearchFieldGrid className="xl:grid-cols-2">
          <SearchTextField
            ...
            onChange={(value) => setSearchFilters((prev) => ({ ...prev, typeCode: value }))}
