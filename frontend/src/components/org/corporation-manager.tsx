...
type CorporationRow = OrganizationCorporationItem & {
  _status: RowStatus;
  _original?: Record<string, unknown>;
  _prevStatus?: RowStatus;
};

const TRACKED_FIELDS: (keyof OrganizationCorporationItem)[] = [
  "enter_cd",
  "company_code",
  "corporation_name",
  "corporation_number",
...
const I18N = {
  title: "법인관리",
  loading: "법인 데이터를 불러오는 중...",
  loadError: "법인 목록을 불러오지 못했습니다.",
  ...
  colEnterCd: "ENTER_CD",
  colCompanyCode: "회사코드",
  colCorporationName: "법인명",
  colCorporationNumber: "법인번호",
  colBusinessNumber: "사업자번호",
  colCompanySeal: "회사 인장",
  colCertificateSeal: "증명서 직인",
  colCompanyLogo: "회사 로고",
  colActive: "사용",
  colUpdatedAt: "수정일",
};
...
function snapshotOriginal(row: OrganizationCorporationItem): Record<string, unknown> {
  return snapshotFields(row, TRACKED_FIELDS);
}

function isRevertedToOriginal(row: CorporationRow): boolean {
  return isRowRevertedToOriginal(row, TRACKED_FIELDS);
}

function toGridRow(item: OrganizationCorporationItem): CorporationRow {
  return {
    ...item,
    corporation_number: item.corporation_number ?? "",
    business_number: item.business_number ?? "",
    company_seal_url: item.company_seal_url ?? "",
    certificate_seal_url: item.certificate_seal_url ?? "",
...
export function CorporationManager() {
  const { can, loading: menuActionLoading } = useMenuActions("/org/corporations");
  const [rows, setRows] = useState<CorporationRow[]>([]);
  ...
  const gridApiRef = useRef<GridApi<CorporationRow> | null>(null);
  const containerRef = useRef<HTMLDivElement | null>(null);
  const tempIdRef = useRef(-1);
  const rowsRef = useRef<CorporationRow[]>([]);
  ...
  const getRowKey = useCallback((row: CorporationRow) => String(row.id), []);

  const applyGridTransaction = useCallback(
    (prevRows: CorporationRow[], nextRows: CorporationRow[]) => {
      const api = gridApiRef.current;
      if (!api) return;

      const prevMap = new Map(prevRows.map((row) => [getRowKey(row), row]));
      const nextMap = new Map(nextRows.map((row) => [getRowKey(row), row]));
      const add: CorporationRow[] = [];
      const update: CorporationRow[] = [];
      const remove: CorporationRow[] = [];
  ...
  const commitRows = useCallback(
    (updater: (prevRows: CorporationRow[]) => CorporationRow[]) => {
  ...
  const fetchCorporations = useCallback(async (filters: SearchFilters, pageNo: number) => {
    const params = new URLSearchParams();
    params.set("page", String(pageNo));
    params.set("limit", String(pageSize));
    if (filters.enterCd.trim()) params.set("enter_cd", filters.enterCd.trim());
    if (filters.companyCode.trim()) params.set("company_code", filters.companyCode.trim());
    if (filters.corporationName.trim()) params.set("corporation_name", filters.corporationName.trim());

    const endpoint =
      params.size > 0 ? `/api/org/corporations?${params.toString()}` : "/api/org/corporations";
    const response = await fetch(endpoint, { cache: "no-store" });
    if (!response.ok) {
      const json = (await response.json().catch(() => null)) as unknown;
      throw new Error(stringifyErrorDetail(json) ?? I18N.loadError);
    }
    return (await response.json()) as OrganizationCorporationListResponse;
  }, [pageSize]);

  const runQuery = useCallback(async (filters: SearchFilters, pageNo: number) => {
    setLoading(true);
    try {
      const response = await fetchCorporations(filters, pageNo);
      const nextRows = response.corporations.map(toGridRow);
      rowsRef.current = nextRows;
      setRows(nextRows);
      setTotalCount(response.total_count ?? response.corporations.length);
  ...
  const defaultColDef = useMemo<ColDef<CorporationRow>>(
    () => ({
      sortable: true,
      filter: true,
      resizable: true,
  ...
  const columnDefs = useMemo<ColDef<CorporationRow>[]>(
    () => [
      {
        ...
        cellRenderer: (params: ICellRendererParams<CorporationRow>) => {
      ...
      {
        headerName: I18N.colEnterCd,
        field: "enter_cd",
        minWidth: 140,
        editable: (params) => params.data?._status !== "deleted",
      },
      {
        headerName: I18N.colCompanyCode,
        field: "company_code",
        minWidth: 140,
        editable: (params) => params.data?._status !== "deleted",
      },
      {
        headerName: I18N.colCorporationName,
      ...
      {
        headerName: I18N.colCorporationNumber,
        field: "corporation_number",
        minWidth: 160,
        editable: (params) => params.data?._status !== "deleted",
      },
      {
        headerName: I18N.colBusinessNumber,
        field: "business_number",
        minWidth: 160,
        editable: (params) => params.data?._status !== "deleted",
      },
      {
        headerName: I18N.colCompanySeal,
        field: "company_seal_url",
        minWidth: 220,
        flex: 1,
        editable: (params) => params.data?._status !== "deleted",
      },
      {
        headerName: I18N.colCertificateSeal,
        field: "certificate_seal_url",
        minWidth: 220,
        flex: 1,
        editable: (params) => params.data?._status !== "deleted",
      },
      {
        headerName: I18N.colCompanyLogo,
  ...
  const getRowClass = useCallback((params: RowClassParams<CorporationRow>) => {
    return getGridRowClass(params.data?._status);
  }, []);
  const rowClassRules = useMemo(() => buildGridRowClassRules<CorporationRow>(), []);

  function addRow() {
    const newId = tempIdRef.current;
    tempIdRef.current -= 1;
    const now = new Date().toISOString();
    const newRow: CorporationRow = {
      id: newId,
      enter_cd: "",
      company_code: "",
      corporation_name: "",
  ...
  function copyRow() {
    ...
    const copied: CorporationRow = {
      ...selectedRow,
      id: newId,
      enter_cd: `${selectedRow.enter_cd}_COPY`,
      company_code: `${selectedRow.company_code}_COPY`,
      created_at: now,
      updated_at: now,
      _status: "added",
      _original: undefined,
  ...
  async function downloadTemplate() {
    ...
    const sheet = XLSX.utils.aoa_to_sheet([
      ...
      [
        ...
        "/assets/certificate-seal.png",
        "/vibehr_mark.svg",
        "Y",
      ],
    ]);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, sheet, "법인관리");
    XLSX.writeFile(workbook, "org-corporations-template.xlsx");
  }

  function uploadTemplate() {
    toast.info("업로드는 다음 단계에서 파일 매핑과 연결됩니다.");
  }
  ...
  const handlePasteCapture = useCallback(
    (event: React.ClipboardEvent<HTMLDivElement>) => {
      ...
      const now = new Date().toISOString();
      const pastedRows: CorporationRow[] = lines.map((line) => {
        const cols = line.split("\t").map((cell) => cell.trim());
        const rowId = tempIdRef.current;
        tempIdRef.current -= 1;
        return {
          id: rowId,
          enter_cd: normalizeCode(cols[0] ?? ""),
          company_code: normalizeCode(cols[1] ?? ""),
          corporation_name: cols[2] ?? "",
          corporation_number: cols[3] ?? "",
          business_number: cols[4] ?? "",
          company_seal_url: cols[5] ?? "",
          certificate_seal_url: cols[6] ?? "",
          company_logo_url: cols[7] ?? "",
          is_active: (cols[8] ?? "Y").toUpperCase() !== "N",
          created_at: now,
          updated_at: now,
          _status: "added",
        };
  ...
  async function downloadXlsx() {
    const exportRows = rows.map((row, index) => ({
      ...
      ENTER_CD: row.enter_cd,
      회사코드: row.company_code,
      법인명: row.corporation_name,
      법인번호: row.corporation_number ?? "",
      사업자번호: row.business_number ?? "",
      회사인장: row.company_seal_url ?? "",
      증명서직인: row.certificate_seal_url ?? "",
      회사로고: row.company_logo_url ?? "",
    ...
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, sheet, "법인관리");
    XLSX.writeFile(workbook, "org-corporations.xlsx");
  }
  ...
  async function saveAll() {
    ...
    for (const row of [...toInsert, ...toUpdate]) {
      if (!normalizeCode(row.enter_cd) || !normalizeCode(row.company_code) || !row.corporation_name.trim()) {
        toast.error("ENTER_CD, 회사코드, 법인명은 필수입니다.");
        return;
      }
    }
    ...
    try {
      await runConcurrentOrThrow(
        ...
        toDelete.map((row) => async () => {
          const response = await fetch(`/api/org/corporations/${row.id}`, { method: "DELETE" });
          if (!response.ok) throw new Error(`삭제 실패: ${row.enter_cd}`);
        }),
        6,
      );
      ...
      await runConcurrentOrThrow(
        ...
        toUpdate.map((row) => async () => {
          const response = await fetch(`/api/org/corporations/${row.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              enter_cd: normalizeCode(row.enter_cd),
              company_code: normalizeCode(row.company_code),
              corporation_name: row.corporation_name.trim(),
              corporation_number: row.corporation_number ?? "",
              business_number: row.business_number ?? "",
              company_seal_url: row.company_seal_url ?? "",
              certificate_seal_url: row.certificate_seal_url ?? "",
          ...
          if (!response.ok) {
            const json = (await response.json().catch(() => null)) as unknown;
            throw new Error(stringifyErrorDetail(json) ?? `수정 실패: ${row.enter_cd}`);
          }
        }),
        6,
      );
      ...
      await runConcurrentOrThrow(
        ...
        toInsert.map((row) => async () => {
          const response = await fetch("/api/org/corporations", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              enter_cd: normalizeCode(row.enter_cd),
          ...
          if (!response.ok) {
            const json = (await response.json().catch(() => null)) as unknown;
            throw new Error(stringifyErrorDetail(json) ?? `입력 실패: ${row.enter_cd || "(신규)"}`);
          }
        }),
        6,
      );
  ...
  const onGridReady = useCallback((event: GridReadyEvent<CorporationRow>) => {
    gridApiRef.current = event.api;
  }, []);

  const onCellValueChanged = useCallback(
    (event: CellValueChangedEvent<CorporationRow>) => {
      if (event.newValue === event.oldValue) return;
      const rowId = event.data?.id;
      const field = event.colDef.field as keyof CorporationRow | undefined;
      if (rowId == null || !field) return;

      commitRows((prev) =>
        prev.map((row) => {
          if (row.id !== rowId) return row;
          const next = { ...row } as CorporationRow;
          if (field === "enter_cd" || field === "company_code") {
            next[field] = normalizeCode(String(event.newValue ?? "")) as never;
    ...
    <ManagerPageShell containerRef={containerRef} onPasteCapture={handlePasteCapture}>
      ...
      >
        <SearchFieldGrid className="xl:grid-cols-3">
          <SearchTextField
            value={searchFilters.enterCd}
            onChange={(value) => setSearchFilters((prev) => ({ ...prev, enterCd: value }))}
            onKeyDown={handleSearchFieldEnter}
            placeholder="ENTER_CD"
          />
          <SearchTextField
          ...
          />
          <SearchTextField
            value={searchFilters.corporationName}
            onChange={(value) => setSearchFilters((prev) => ({ ...prev, corporationName: value }))}
            onKeyDown={handleSearchFieldEnter}
            placeholder="법인명"
          />
        </SearchFieldGrid>
