...
export async function GET(request: NextRequest) {
  ...
  const upstreamUrl = new URL(`${backendApiBaseUrl()}/api/v1/org/mapping-type-items`);
...
export async function POST(request: NextRequest) {
  ...
  const upstreamResponse = await fetch(`${backendApiBaseUrl()}/api/v1/org/mapping-type-items`, {
    method: "POST",
    headers: {
