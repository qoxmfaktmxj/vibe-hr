...
export async function GET(request: NextRequest) {
  ...
  const upstream = await fetch(`${backendApiBaseUrl()}/api/v1/hr/appointments/records${search}`, {
    method: "GET",
...
export async function POST(request: NextRequest) {
  ...
  const upstream = await fetch(`${backendApiBaseUrl()}/api/v1/hr/appointments/records`, {
    method: "POST",
    headers: {
