...
- AG Grid 화면 수정 전 아래 순서를 반드시 읽을 것:
  1. `config/grid-screens.json`
  2. `docs/GRID_SCREEN_STANDARD.md`
  3. `docs/AG_GRID_COMMON_GUIDE.md`
  4. 대상 화면 page/component 파일
- `AgGridReact` 사용 화면은 예외 없이 다음을 준수:
  - `frontend/src/components/grid/*`, `frontend/src/lib/grid/*` 공통 모듈 사용
  - 표준 toolbar 순서 유지: `query -> create -> copy -> template -> upload -> save -> download`
  - page 파일에 `GRID_SCREEN` 메타데이터 선언
  - `config/grid-screens.json` 등록/갱신
  - `npm run validate:grid`를 lint/build 전에 실행
...
- Large refactors must be sliced by screen, endpoint, or domain unit. Avoid broad rewrites across unrelated modules.

### Planner-first workflow
- Start broad work with architecture or planning prompts before executor prompts.
- Use team execution only after scope, acceptance criteria, and affected files are identified.
- Prefer one vertical slice at a time: screen + API + QA + docs.
...
| "vibe-validate", "validate all", "full check", "전체검증" | `$vibe-validate` | Read `./.codex/skills/vibe-validate/SKILL.md`, run full validation pipeline |
| "vibe-payroll", "payroll change", "급여변경", "급여수정" | `$vibe-payroll` | Read `./.codex/skills/vibe-payroll/SKILL.md`, enforce R3 payroll safety |
| "vibe-grid", "grid change", "그리드수정", "AG Grid" | `$vibe-grid` | Read `./.codex/skills/vibe-grid/SKILL.md`, enforce AG Grid standards |
| "vibe-permission", "permission change", "권한변경", "메뉴권한" | `$vibe-permission` | Read `./.codex/skills/vibe-permission/SKILL.md`, enforce permission safety |
...
| --- | --- | --- | --- |
| Frontier (leader) | `gpt-5.3-codex` | high | Primary leader/orchestrator for planning, coordination, and frontier-class reasoning. |
| Spark (explorer/fast) | `gpt-5.3-codex-spark` | low | Fast triage, explore, lightweight synthesis, and low-latency routing. |
| Standard (subagent default) | `gpt-5.3-codex` | high | Default standard-capability model for installable specialists and secondary worker lanes unless a role is explicitly frontier or spark. |
| `explore` | `gpt-5.3-codex-spark` | low | Fast codebase search and file/symbol mapping (fast-lane, fast) |
| `analyst` | `gpt-5.3-codex` | medium | Requirements clarity, acceptance criteria, hidden constraints (frontier-orchestrator, frontier) |
| `planner` | `gpt-5.3-codex` | medium | Task sequencing, execution plans, risk flags (frontier-orchestrator, frontier) |
| `architect` | `gpt-5.3-codex` | high | System design, boundaries, interfaces, long-horizon tradeoffs (frontier-orchestrator, frontier) |
| `debugger` | `gpt-5.3-codex` | high | Root-cause analysis, regression isolation, failure diagnosis (deep-worker, standard) |
| `executor` | `gpt-5.3-codex` | high | Code implementation, refactoring, feature work (deep-worker, standard) |
| `team-executor` | `gpt-5.3-codex` | medium | Supervised team execution for conservative delivery lanes (deep-worker, frontier) |
| `verifier` | `gpt-5.3-codex` | high | Completion evidence, claim validation, test adequacy (frontier-orchestrator, standard) |
| `style-reviewer` | `gpt-5.3-codex-spark` | low | Formatting, naming, idioms, lint conventions (fast-lane, fast) |
| `quality-reviewer` | `gpt-5.3-codex` | medium | Logic defects, maintainability, anti-patterns (frontier-orchestrator, standard) |
| `api-reviewer` | `gpt-5.3-codex` | medium | API contracts, versioning, backward compatibility (frontier-orchestrator, standard) |
| `security-reviewer` | `gpt-5.3-codex` | medium | Vulnerabilities, trust boundaries, authn/authz (frontier-orchestrator, frontier) |
| `performance-reviewer` | `gpt-5.3-codex` | medium | Hotspots, complexity, memory/latency optimization (frontier-orchestrator, standard) |
| `code-reviewer` | `gpt-5.3-codex` | high | Comprehensive review across all concerns (frontier-orchestrator, frontier) |
| `dependency-expert` | `gpt-5.3-codex` | high | External SDK/API/package evaluation (frontier-orchestrator, standard) |
| `test-engineer` | `gpt-5.3-codex` | medium | Test strategy, coverage, flaky-test hardening (deep-worker, frontier) |
| `quality-strategist` | `gpt-5.3-codex` | medium | Quality strategy, release readiness, risk assessment (frontier-orchestrator, standard) |
| `build-fixer` | `gpt-5.3-codex` | high | Build/toolchain/type failures resolution (deep-worker, standard) |
| `designer` | `gpt-5.3-codex` | high | UX/UI architecture, interaction design (deep-worker, standard) |
| `writer` | `gpt-5.3-codex` | high | Documentation, migration notes, user guidance (fast-lane, standard) |
| `qa-tester` | `gpt-5.3-codex` | low | Interactive CLI/service runtime validation (deep-worker, standard) |
| `git-master` | `gpt-5.3-codex` | high | Commit strategy, history hygiene, rebasing (deep-worker, standard) |
| `code-simplifier` | `gpt-5.3-codex` | high | Simplifies recently modified code for clarity and consistency without changing behavior (deep-worker, frontier) |
| `researcher` | `gpt-5.3-codex` | high | External documentation and reference research (fast-lane, standard) |
| `product-manager` | `gpt-5.3-codex` | medium | Problem framing, personas/JTBD, PRDs (frontier-orchestrator, standard) |
| `ux-researcher` | `gpt-5.3-codex` | medium | Heuristic audits, usability, accessibility (frontier-orchestrator, standard) |
| `information-architect` | `gpt-5.3-codex` | low | Taxonomy, navigation, findability (frontier-orchestrator, standard) |
| `product-analyst` | `gpt-5.3-codex` | low | Product metrics, funnel analysis, experiments (frontier-orchestrator, standard) |
| `critic` | `gpt-5.3-codex` | high | Plan/design critical challenge and review (frontier-orchestrator, frontier) |
| `vision` | `gpt-5.3-codex` | low | Image/screenshot/diagram analysis (fast-lane, frontier) |
<!-- OMX:MODELS:END -->
